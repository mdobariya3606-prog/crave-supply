...
<body>@include('layouts.header')<main class="deleted-page">
        <div class="deleted-head">
            <div>
                <p>Customer management</p>
                <h1>Deleted customers</h1>
                <p>Restore accounts or permanently remove them.</p>
            </div>@if($deletedCustomers->isNotEmpty())<div class="deleted-actions">
                ...
                <tbody>@endif<tr>
                        ...
                        <td>
                            <div class="row-actions">
                                <form method="POST" action="{{ route('admin.customers.restore', $customer->id) }}">@csrf @method('PATCH')<button type="submit">Restore</button></form>
                                <form method="POST" action="{{ route('admin.customers.force-destroy', $customer->id) }}" onsubmit="return confirm('Permanently delete this customer?')">@csrf @method('DELETE')<button class="danger" type="submit">Delete</button></form>
                            </div>
                        </td>
                    </tr>@if($loop->last)</tbody>
            </table>@endif @empty<div class="empty-deleted">No deleted customer accounts.</div>@endforelse</div>{{ $deletedCustomers->links() }}

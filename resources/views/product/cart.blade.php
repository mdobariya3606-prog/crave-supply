...
<body>
    ...
    <main class="cart-page">
        ...
        @if (session('success'))
            <div class="cart-alert" role="status">{{ session('success') }}</div>
        @endif
        @if (empty($cart))
            <div class="cart-empty">
                Your cart is empty.
                <a href="{{ route('products.dashboard') }}">Browse products</a>.
            </div>
        @else
                ...
                <section class="cart-card" aria-label="Cart items">
                    @foreach ($cart as $item)
                        ...
                        >
                            ...
                            <form
                                class="cart-quantity"
                                action="{{ route('cart.update', $item['slug'] ?? $item['product_id']) }}"
                                method="POST"
                            >
                                @csrf
                                @method ('PUT')
                            ...
                            <form
                                action="{{ route('cart.remove', $item['slug'] ?? $item['product_id']) }}"
                                method="POST"
                            >
                                <input
                                    type="hidden"
                                ...
                                />
                                @method ('DELETE')
                                <button class="cart-remove" type="submit">
                                    Remove
                                </button>
                            </form>
            ...
            (() => {
                ...
                const refresh = (input) => {
                    ...
                    document.querySelector("[data-cart-subtotal]").textContent =
                        money(total);
                    const deliveryNote = document.querySelector(
                        "[data-delivery-note]",
                    );
